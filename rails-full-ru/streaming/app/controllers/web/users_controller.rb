...
class Web::UsersController < Web::ApplicationController
  ...
  def stream_csv
    ...
    send_stream(filename: "stream_scv.csv") do |stream|
      stream.write "#{User.column_names.join(',')}\n"

      User.all.find_each do |record|
        p "record = #{User.column_names}"
        stream.write "#{record.attributes.values_at(*User.column_names).join(',')}\n"
      end
    end
  end
